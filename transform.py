...
NORM_POINTS = (RIGHT_EYE_POINTS + LEFT_EYE_POINTS + JAW_POINTS)
...
def faceswap(face_image, face_landmarks, target_image, target_landmarks):
    M = transformation_from_points(target_landmarks[ALIGN_POINTS],
                                   face_landmarks[ALIGN_POINTS])
    
    mask = get_face_mask(face_image, face_landmarks)
    warped_mask = warp_im(mask, M, target_image.shape)
    combined_mask = np.max([get_face_mask(target_image, target_landmarks), warped_mask],
                              axis=0)
    
    warped_target_image = warp_im(face_image, M, target_image.shape)
    warped_corrected_target_image = correct_colours(target_image, warped_target_image, target_landmarks)
